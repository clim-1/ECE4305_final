...
void DdfsCore::init() {
   ...
   set_pha_source(0);
   // set note C
   set_carrier_freq(0,0);
   set_carrier_freq(0,1);
   set_carrier_freq(0,2);
   set_carrier_freq(0,3);
   set_carrier_freq(0,4);
   set_carrier_freq(0,5);
   set_carrier_freq(0,6);
   set_carrier_freq(0,7);
...
void DdfsCore::set_carrier_freq(int freq, int track) {
   uint32_t fcw, p2n;
   float tmp;

   io_write(base_addr, TRK_SEL, track);
